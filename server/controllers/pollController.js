...
const postPoll = async ( req, res) => {
  ...
  if (!ownerId || !title || !options) return req.status(404).json({error: "Invalid Request"}); // replace with 404 Error
...
const postVote = async (req, res) => {
  ...
  if (!pollId || !optionId) return req.status(404).json({error: "Invalid Request"}); // update for 400 Error
  ...
  if (updateOption.modifiedCount == 0) return req.status(404).json({error: "Invalid Request"}); // update for 400 Error
